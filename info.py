...
BOT_TOKEN = environ.get('BOT_TOKEN', "6255835766:AAG03-Ht3c8PJI02QNF5MNx91Lqh4SJnQMI")
...
STREAMHTO = (environ.get('STREAMHTO', 'https://t.me/howtorfe/7'))

# Admins, Channels & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1790510761').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002092061539').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002092061539') #Channel / Group Id for force sub ( make sure bot is admin )
...
DATABASE_NAME = environ.get('DATABASE_NAME', "Tests")
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002092061539')) #Log channel id ( make sure bot is admin )
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002092061539')).split()]
